...
import (
	...
	"gitlab.com/Raven-IO/GoSymTable/profile"
	pb "gitlab.com/Raven-IO/GoSymTable/protogen/go/metastore"
	"gitlab.com/Raven-IO/GoSymTable/symbol/demangle"
)
	...
	if entry != nil {
		for _, field := range entry.Field {
			if field.Attr == dwarf.AttrName {
				// by definition, entry.Field is a one element slice,
				// we can return from the function immediately
				switch name := field.Val.(type) {
				case string:
					return name
				default:
					return "?"
